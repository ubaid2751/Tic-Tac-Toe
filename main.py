...
class AI:
    ...
    def minimax(self, board, maximizing):
        ...
        if maximizing:
            ...
            for (row, col) in empty_sqrs:
                ...
                evl = self.minimax(temp_board, False)[0]
                if evl > max_eval:
                    max_eval = evl
                    best_move = (row, col)
            ...
            for (row, col) in empty_sqrs:
                ...
                evl = self.minimax(temp_board, True)[0]
                if evl < min_eval:
                    min_eval = evl
                    best_move = (row, col)
    ...
    def eval(self, main_board):
        if self.level == 0:
            ...
            evl, move = self.minimax(main_board, False)

        print(f'AI has chosen to mark the square in pos {move} with an eval of: {evl}')
